// BE-024: Unit tests for the user_roles join table
// Requires a disposable test database pointed to by DATABASE_URL.
// Run with: npx jest tests/user-role.test.ts
//
// These tests assert on Prisma's known error codes rather than parsing
// raw Postgres error text, since that's what application code will
// actually branch on (P2002 = unique constraint, P2003 = FK constraint).
