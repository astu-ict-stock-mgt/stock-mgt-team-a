...
function PlaceholderScreen({ title }: { title: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-[#94A3B8] mt-1">This screen will be implemented by the frontend team.</p>
    </div>
  )
}
...
export default function App() {
  ...
  if (!isLoggedIn) {
    return (
      <>
        <LoginScreen onLogin={() => { setIsLoggedIn(true); toast.success('Welcome back, Marcus!') }} />
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden">
      {/* SIDEBAR */}
      <aside className={`flex flex-col bg-[#0F172A] border-r border-[#1E293B] transition-all duration-200 ${sidebarCollapsed ? 'w-14' : 'w-56'} shrink-0 print:hidden`}>
        <div className={`flex items-center gap-3 border-b border-[#1E293B] bg-gradient-to-b from-white/[0.02] to-transparent ${sidebarCollapsed ? 'px-3 py-4 justify-center' : 'px-4 py-4'}`}>
          ...
          {navGroups.map((group, gi) => (
            <div key={gi} className={gi > 0 ? 'mt-4' : ''}>
              {!sidebarCollapsed && group.label && (
                <p className="px-2 py-1 text-[10px] font-semibold text-[#475569] uppercase tracking-widest mb-1">{group.label}</p>
              )}
                ...
                return (
                  ...
                  >
                    ...
                    {item.badge && !sidebarCollapsed && (
                      <span className="ml-auto px-1.5 py-0.5 bg-[#DC2626] text-white text-[10px] rounded-full font-bold">{item.badge}</span>
                    )}
                    {item.badge && sidebarCollapsed && (
                      <span className="absolute top-0.5 right-0.5 w-3.5 h-3.5 bg-[#DC2626] text-white text-[9px] rounded-full flex items-center justify-center font-bold">{item.badge}</span>
                    )}
                  </button>
                )
              })}
        ...
        <div className={`border-t border-[#1E293B] p-2 space-y-0.5`}>
          <button onClick={() => navigate('notifications')}
            ...
            {unreadNotifications > 0 && (
              <span className={`${sidebarCollapsed ? 'absolute top-0.5 right-0.5 w-4 h-4 text-[9px]' : 'ml-auto px-1.5 py-0.5 text-[10px]'} bg-[#DC2626] text-white rounded-full flex items-center justify-center font-bold`}>
                {unreadNotifications}
              </span>
            )}
          </button>
      ...
      <div className="flex-1 flex flex-col min-w-0 print:block">
        ...
        <header className="h-14 bg-white border-b border-[#E2E8F0] flex items-center justify-between px-4 shrink-0 print:hidden shadow-[0_1px_2px_0_rgb(0,0,0,0.03)]">
          <SearchBar value={globalSearch} onChange={setGlobalSearch} placeholder="Search anything..." className="w-64" />
          <div className="flex-1" />

          {/* Quick stats */}
          <div className="hidden lg:flex items-center gap-4 text-xs text-[#94A3B8] border-r border-[#E2E8F0] pr-4 mr-1">
            <span><span className="text-[#D97706] font-semibold">{lowStockCount}</span> low stock</span>
            <span><span className="text-[#4F46E5] font-semibold">5</span> pending</span>
          </div>

          {/* Notification bell */}
          <button onClick={() => navigate('notifications')} className="relative w-9 h-9 rounded-lg hover:bg-[#F1F5F9] flex items-center justify-center text-[#64748B] hover:text-[#1E293B] transition-colors">
            {Icons.notifications}
            {unreadNotifications > 0 && (
              <span className="absolute top-1 right-1 w-4 h-4 bg-[#DC2626] text-white text-[9px] rounded-full flex items-center justify-center font-bold">{unreadNotifications}</span>
            )}
          </button>
          ...
          <div className="relative">
            <button onClick={() => setUserMenuOpen(o => !o)}
              ...
              <div className="w-7 h-7 rounded-full bg-[#EEF2FF] text-[#4F46E5] text-xs font-semibold flex items-center justify-center">MT</div>
              <div className="hidden sm:block text-left">
                <p className="text-xs font-semibold text-[#1E293B] leading-none">Marcus Thompson</p>
                <p className="text-[10px] text-[#94A3B8] mt-0.5">Administrator</p>
              </div>
              <svg className="w-3 h-3 text-[#94A3B8]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6" /></svg>
            </button>
            {userMenuOpen && (
              <div className="absolute right-0 top-full mt-1.5 w-52 bg-white rounded-xl border border-[#E2E8F0] shadow-lg z-30 py-1 overflow-hidden">
                <div className="px-3 py-2.5 border-b border-[#F1F5F9]">
                  <p className="text-xs font-semibold text-[#1E293B]">Marcus Thompson</p>
                  <p className="text-[11px] text-[#94A3B8]">mthompson@stockmanager.io</p>
                </div>
                {[
                  { label: 'My profile', icon: Icons.users, action: () => navigate('settings') },
                  { label: 'Notifications', icon: Icons.notifications, action: () => navigate('notifications') },
                  ...
                  <button key={i} onClick={item.action}
                    ...
                    <span className="text-[#94A3B8]">{item.icon}</span>{item.label}
                  </button>
                ))}
        ...
        <main className="flex-1 overflow-auto bg-[#F8FAFC] print:bg-white print:overflow-visible">
          <div className="p-6 max-w-[1400px] mx-auto min-h-full print:p-0 print:max-w-none">
            ...
            {screen === 'stock-issuing' && <PlaceholderScreen title="Stock Issuing" />}
            {screen === 'stock-transfer' && <PlaceholderScreen title="Stock Transfer" />}
            {screen === 'stock-tracking' && <PlaceholderScreen title="Stock Tracking" />}
